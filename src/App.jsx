...
function App() {
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar"><div className="brand"><div className="brand-mark"><Sparkles size={17} /></div><span>pharm<span>track</span></span></div><div className="profile"><div className="avatar">AM</div><div><strong>Alex Morgan</strong><small>M.Pharm · Year 1</small></div><MoreHorizontal size={17} /></div><p className="nav-label">Workspace</p><nav>{nav.map(([label, Icon]) => <button key={label} className={activeView === label ? 'active' : ''} onClick={() => setActiveView(label)}><Icon size={18} />{label}<span>{label === 'Daily log' ? tasks.length : label === 'Literature' ? literature.length : ''}</span></button>)}</nav><div className="sidebar-bottom"><div className="course-mini"><div className="mini-row"><span>Course progress</span><strong>42%</strong></div><div className="progress"><i style={{ width: '42%' }} /></div><small>Ends December 2027</small></div><button className="settings"><Settings2 size={17} />Settings</button></div></aside>
      <main className="main-content"><header className="topbar"><div><p className="eyebrow">{today.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</p><h1>{activeView === 'Overview' ? 'Good morning, Alex' : activeView}</h1></div><div className="top-actions"><button className="icon-button"><Search size={18} /></button><button className="button primary" onClick={() => setShowTaskForm(true)}><Plus size={17} /> Add task</button><div className="top-avatar">AM</div></div></header>
